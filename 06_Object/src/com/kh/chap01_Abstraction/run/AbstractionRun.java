...
public class AbstractionRun {
	...
	/*
	 ...
	 5. 추상화 과정
	 	...
	 	4) 최종적으로 추려진 속성들을 가지고 어떤 자료형or 어떤 변수명으로 사용할 건지 생각
	 			...
	 			2) "배열" 만으로 프로그램을 구현하게 된다면?
	 				* 배열 - 하나의 자료형의 여러개의 값들을 보관할 수 있음
	 				...
	 				--> 어떤 문제가 발생? --> 한명이 전학가게 된다면 해당 학생의 인덱스를 찾아서 일일히 name, age, height 배열속에 찾아서 지워줘야되는데
	 									...
	 									--> 새로운 학생이 오게되면 새로운 정보를 배열에 추가해줘야하는데 크기변경 불가하다(배열의 특징상 )
	 									
	 			3) 그래서 나온 개념이 "구조체"의 개념!
	 			
	 				* 구조체 - 여러개의 자료형의 여러개의 값들을 보관할 수 있는 배열같은 형태 ( 직접 자료형을 만든다 == 클래스 )(나만의클래스라는 자료형)
	...
	public static void main(String[] args) {
		...
		//이런식으로 직접적으로 접근이 가능하면=> 외부에서 무분별하게 직접접근을 통해 값을 변질시킨다거나 정보들을 알아낼수가있다.
		// => 캡슐화 작업을 해야된다
		// => 객체지향 설계원칙 중에 정보은닉-> 그리고 그 정보은닉중 하나의 방법이 캡슐화다.
		//-> 캡슐화는 chap02패키지에서 설명계속.
	}
